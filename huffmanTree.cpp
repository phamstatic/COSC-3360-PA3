...
struct Node { // Holds information needed for the Huffman Tree's root and nodes.
    char symbol = ' ';
    int frequency = -1;
    Node *next = nullptr;
    bool sum = false;
...
class PriorityQueue { // Contains all the parameters and functions needed to properly generate the Huffman Tree. 
private:
    Node *front;
    int size;
public:
    ...
    void printQueue() { // Prints the queue if needed for debugging.
        if (front == nullptr) {
            return;
        }
        else {
